...
public class GTFFileParser extends BEDFileParser {
	...
	public static void main(String[] args) throws IOException{
		...
		GTFFileParser dummy = new GTFFileParser(args[0]);
	}
			...
			if(looksLikeData(nextLine, data, chr) && (nextLine.trim().length() > 0)){
				...
				if(currentExtraFields.isEmpty() || !fieldMap.get("transcript_id").equals(currentExtraFields.get("transcript_id"))) {
					if(!currentExtraFields.isEmpty()) {
						...
						//}
						//else {
							RefSeqGene g = createRefSeqFromRawGTF(currentExons, currentExtraFields);
	...
	private static RefSeqGene createRefSeqFromRawGTF(
		...
		RefSeqGene g = new RefSeqGene(currentExons);
		g.setName(name);
	 ...
	 * @throws IOException 
	 **/
	public void GTFWrite(BufferedWriter bw, String Src, boolean UseExtraFields) throws IOException {
		
		if (!UseExtraFields)
			bed2CuffGtf(bw,Src,false);
		else {
			for (RefSeqGene gi : this.GetGenes()){
				String str;
