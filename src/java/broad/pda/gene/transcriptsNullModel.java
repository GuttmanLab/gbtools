	...
	private void removeRegionFromIntergenicGenome(BEDFileParser refGenes) {
		...
		while(chrIt.hasNext()){
			...
			if (! intergenicGenome.containsKey(chr))
				continue;
			IntervalTree<RefSeqGeneWithIsoforms> refTree=refGenes.getChrTree(chr);
			IntervalTree<Alignments> intergenicTree= this.intergenicGenome.get(chr);
			
			Iterator<RefSeqGeneWithIsoforms> refIt=refTree.valueIterator();
			while (refIt.hasNext()){
				
				for (RefSeqGene currentElement : refIt.next().getAllIsoforms()){
    		...
    		for (RefSeqGene g:  ref.GetGenes()){
    			...
    			while(addedGen==false){
	    			...
	    			if (delta >= g.getGenomicLength()){
	    				...
	    				RefSeqGene newGen = new RefSeqGene (chr,startPos,startPos+ g.getGenomicLength());
	    				newGen.setName(g.getName());
	    				newGen.setOrientation (g.getOrientation());
	    				Alignments[] new_exons = new Alignments[ g.getNumExons()];
	    				Alignments[] old_exons = g.getExons();
	    				for (int j=0; j <g.getNumExons(); j++) {
	    					
	    					int nst = startPos+ (old_exons[j].getStart() - g.getStart());
	    					int nend =startPos+ (old_exons[j].getEnd() - g.getStart());
	    					new_exons[j] = new Alignments (chr,nst,nend);
	    				
	    				}
	    				newGen.setExons(new_exons,null );
	...
	public void writeIntergenicGenome(String outf) throws IOException {
		...
		for(String chr: intergenicGenome.keySet()){
			...
			while(it.hasNext()){
				...
				RefSeqGene g = new RefSeqGene(aln);
				bw.write(g.toBED()+"\n");
			}
