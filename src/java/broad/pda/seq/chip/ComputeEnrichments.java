...
public class ComputeEnrichments {
	...
	public ComputeEnrichments(File[] files, Collection<Alignments> regions, String save, String sizes)throws IOException{
		Map<Alignments, double[]>[] scoreArray=new Map[files.length];
		...
		for(int i=0; i<files.length; i++){
			if(!files[i].getName().endsWith("sai")){
				...
				Map<Alignments, double[]> scores=model.scoreSegments(regions);
				scoreArray[i]=scores;
	...
	private void write(String save, Map<Alignments, double[]> [] scores, ArrayList<String> names)throws IOException{
		...
		for(Alignments align: scores[0].keySet()){
	...
	public static void main(String[] args)throws IOException{
		if(args.length>3){
			...
			Collection<Alignments> regions=BEDFileParser.loadAlignmentData(new File(args[1]));
			String save=args[2];
			String sizes=args[3];
