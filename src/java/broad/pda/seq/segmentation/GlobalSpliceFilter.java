...
public class GlobalSpliceFilter {
	...
	public GlobalSpliceFilter(AlignmentDataModel data, String save, String genomeDirectory, String chr)throws IOException{
		Map<Alignments, double[]> map=filterSpliceSites(data, genomeDirectory, chr);
	...
	private Map<Alignments, double[]> filterSpliceSites(AlignmentDataModel data, String genomeDirectory, String chr) throws IOException {
		...
		//for(String chr: data.getChromosomeLengths().keySet()){
			...
			Map<Alignments, Integer> spliced=data.getSplicedReads(chrRegion);
			if(!spliced.isEmpty()){
	...
	private Map<Alignments, double[]> score(Map<Alignments, Integer> spliced, double lambda, String genomeDir, String chr) throws IOException {
		Map<Alignments, double[]> rtrn=new TreeMap();
