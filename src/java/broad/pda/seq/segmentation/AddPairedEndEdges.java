...
public class AddPairedEndEdges {
	...
	private void write(String save, ChromosomeWithBubblesJGraphT graph)throws IOException{
		FileWriter writer=new FileWriter(save);
	...
	public static Collection<Path> addPairedEndEdgesToPaths(ChromosomeWithBubblesJGraphT graph, AlignmentDataModelStats pairedData, String chr, int start, int end, String saveToFile) throws IOException {
		Collection<Path> paths=graph.getAllPaths();
		//writePaths(paths, saveToFile + ".allPaths.bed");
		...
		try {
			pairedDist.write(saveToFile + "."+chr+".paired.size.dist.txt");
		...
		*/
		...
		/*
		writePaths(plusPathTree, saveToFile + ".pluspathtree.bed");
		writePaths(minusPathTree, saveToFile + ".minuspathtree.bed");
		writePaths(paths, saveToFile + ".rtrn01.bed");
		writePaths(graph.getPaths(1), saveToFile + ".rtrn0.bed");
		writePaths(graph.getAllPaths(), saveToFile + ".allpathsatbegin.bed");
		*/
		
		joinPathsEfficient(graph, paths, plusPathTree, pairedData, chr, start, end, "+", pairedDist);
		
		joinPathsEfficient(graph, paths, minusPathTree, pairedData, chr, start, end, "-", pairedDist);
		try {
			graph.writeGraph(saveToFile + "."+chr+".graph.post.add.pairs.dot", new Alignments(graph.getName(), start, end));
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		//writePaths(graph.getAllPaths(), saveToFile + ".allpathsatend.bed");
		
		Collection<Path> rtrn=graph.getPaths(1);
		//writePaths(rtrn, saveToFile + ".rtrn1.bed");
		rtrn.addAll(graph.getOrphanPaths());
		//writePaths(rtrn, saveToFile + ".rtrn2.bed");
		return rtrn;
	}
	
	public static void writePaths(Collection<Path> paths, String save) {
		try {
			FileWriter fw = new FileWriter(save);
			for (Path path : paths) {
				RefSeqGene g = path.toGene();
				fw.write(g + "\n");
			}
			fw.close();
		}
	...
	private static void writePaths(IntervalTree<Collection<Path>> paths, String save) {
		try {
			FileWriter fw = new FileWriter(save);
			Iterator<Node<Collection<Path>>> i = paths.iterator();
			while (i.hasNext()) {
				Node<Collection<Path>> n = i.next();
				Collection<Path> c = n.getValue();
				for (Path path : c) {
					RefSeqGene g = path.toGene();
					fw.write(g + "\n");
				}
	...
	private static void joinPaths(ChromosomeWithBubblesJGraphT graph, Collection<Path> paths, IntervalTree<Collection<Path>> pathTreeByExons,AlignmentDataModelStats pairedData, String chr, int start, int end, String orientation) throws IOException{
					...
					for(Path path1: paths1){
						for(Path path2: paths2){
							...
							if(!path1.toGene().getAlignment().overlaps(path2.toGene().getAlignment())){
								...
								if(edge != null) {
									boolean b=graph.addEdge(edge.getLeftNode(), edge.getRightNode(), edge);
								}
							}
							counter++;
						}
	...
	private static void joinPathsEfficient(ChromosomeWithBubblesJGraphT graph, Collection<Path> paths, IntervalTree<Collection<Path>> pathTree,AlignmentDataModelStats pairedData, String chr, int start, int end, String orientation, EmpiricalDistribution pairedInsertDistribution) throws IOException{
		...
		for(Path path: paths){
			...
			if(path.getOrientation().equalsIgnoreCase(orientation) || path.getOrientation().equalsIgnoreCase("*")){
				...
				Map<Path, Integer> overlappingPaths=new TreeMap<Path, Integer>();
				Map<Path, Set<Alignments>> readsForOverlappingPaths = new TreeMap<Path, Set<Alignments>>();
						...
						//TODO if read is in the same path then skip it
							if(read.getAlignmentStart() < path.getStart() ){
								...
								for(Path p: paths1){
									...
									Alignments a = new Alignments(read.getChr(), read.getAlignmentStart(), read.getAlignmentEnd());
									if (readsForOverlappingPaths.containsKey(p)) {
										readsForOverlappingPaths.get(p).add(a);
									} else {
										TreeSet<Alignments> ts = new TreeSet<Alignments>();
										ts.add(a);
										readsForOverlappingPaths.put(p, ts);
									}
				...
				for(Path connectedPath: overlappingPaths.keySet()){
					...
					Set<Alignments> pairedReadsForConnectedPath = readsForOverlappingPaths.get(connectedPath);
						...
						if(!connectedPath.equals(path1) && !connectedPath.overlaps(path1)){
							...
							*/
							...
							if(edge != null && pairedCounts > MIN_PAIRED_END_SUPPORT) {
								BubbleEdge old = graph.getEdge(edge.getLeftNode(), edge.getRightNode());
								...
								if(old == null) {
									...
									for (Alignments pairedRead : pairedReadsForConnectedPath) {
										...
										if (leftPathAsGene.hasExon(pairedReadStart) && rightPathAsGene.hasExon(pairedReadEnd)) {
											int insertSize = AddPairedEndEdges.estimateInsertSize(pairedRead, leftPathAsGene, rightPathAsGene);
									...
									//shouldAddEdge = false;
									if (shouldAddEdge)
										couldAdd=graph.addEdge(edge.getLeftNode(), edge.getRightNode(), edge);
	...
	private static BubbleEdge getEdge(Path path, Path connectedPath, String orientation, Integer pairedCount, ChromosomeWithBubblesJGraphT graph) {
		...
		edge.setVertexCount(v1, count1);
		edge.setVertexCount(v2, count2);
	...
	private static void joinPaths(ChromosomeWithBubblesJGraphT graph, IntervalTree<Collection<Path>> pathTree,AlignmentDataModelStats pairedData, String chr, int start, int end, String orientation) throws IOException {
	...
	private static void addToGraph(ChromosomeWithBubblesJGraphT graph, Alignment read, Iterator<Node<Collection<Path>>> overlappers, String orientation, AlignmentDataModelStats  pairedData) throws IOException {
