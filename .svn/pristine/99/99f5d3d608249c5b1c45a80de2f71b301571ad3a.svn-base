...
public class ReconstructionComparer{
	...
	public ReconstructionComparer (String[] args)throws IOException{
		...
		List<RefSeqGene> refGenes = reference.GetGenes();
		List<RefSeqGene> compareGenes = annotations.GetGenes();
		
		//For every reference gene
		for(RefSeqGene gene1:refGenes){
			for(RefSeqGene gene2:compareGenes){
	...
	private boolean passesOverlap(RefSeqGene gene1, RefSeqGene gene2){
