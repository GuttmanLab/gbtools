 ...
 *
 */
public class ChromosomeWithBubblesJGraphT extends DefaultDirectedGraph<LightweightGenomicAnnotation, BubbleEdge>{//, Graph<LightweightGenomicAnnotation, BubbleEdge> {
	 ...
	 */
	public ChromosomeWithBubblesJGraphT (String name, Collection<? extends LightweightGenomicAnnotation> vertices, Collection<? extends LightweightGenomicAnnotation> edges, Map<? extends LightweightGenomicAnnotation, Double> edgeCounts, Map<? extends LightweightGenomicAnnotation, Double> nodeCounts, double lambda, double numMarkers, double numReads, int minSplices) throws IllegalArgumentException{
		...
		for(LightweightGenomicAnnotation vertex : vertices) {
			/* 
			 * Check that the chromosome of the vertex is the same as the chromosome in question 
			 */
			if(!name.equals(vertex.getChromosome())) {
		...
		for(LightweightGenomicAnnotation edge : edges) {
			...
			for(LightweightGenomicAnnotation left : lefts) {
				...
				for(LightweightGenomicAnnotation right : rights) {
					...
					BubbleEdge e = edge.inReversedOrientation() ? getEdge(right, left) : getEdge(left, right); 
	 ...
	 */
	public ChromosomeWithBubblesJGraphT(String name, Map<? extends LightweightGenomicAnnotation, Double> nodes, Map<BubbleEdge, Double> edges, double lambda, double numMarkers, double numReads, double minEdgeSupport){
		this(name);
		this.lambda=lambda;
		this.numberOfMarkers=numMarkers;
		this.numberOfReads=numReads;
	 ...
	 * @param numReads
	 */
	/*public ChromosomeWithBubblesJGraphT(String name, Collection<? extends LightweightGenomicAnnotation> nodes, Collection<BubbleEdge> edges, boolean fromBE, double lambda, double numMarkers, double numReads){
		this(name);
		this.lambda=lambda;
		this.numberOfMarkers=numMarkers;
	 ...
	 */
	public ChromosomeWithBubblesJGraphT (String name, Collection<? extends LightweightGenomicAnnotation> nodes, Collection<? extends LightweightGenomicAnnotation> edges, double lambda, double numMarkers, double numReads, int minSplices) throws IllegalArgumentException{
	 ...
	 * @return
	 */
	public boolean addEdge(LightweightGenomicAnnotation v1, LightweightGenomicAnnotation v2, BubbleEdge e) {
...
//		Collection<LightweightGenomicAnnotation> allSinks = getTargetVertices(getValidTypes());
//		System.out.println("# Targets: "+allSinks.size());
		
		/*
		 * 1. GET ALL CONNECTED COMPONENTS
		 */
		ConnectivityInspector<LightweightGenomicAnnotation, BubbleEdge> CI = new ConnectivityInspector<LightweightGenomicAnnotation, BubbleEdge>(this);
		List<Set<LightweightGenomicAnnotation>> comps = CI.connectedSets();
		//logger.info("# Connected Components: "+comps.size());
		/*
		 * FOR EACH COMPONENT
		 */
		for(Set<LightweightGenomicAnnotation> component: comps){
			...
			if(component.size()>1){
				...
				DirectedSubgraph<LightweightGenomicAnnotation, BubbleEdge>[] strandSpecificSubgraphs = new DirectedSubgraph[2];
				
				// the [0] is the positive subgraph
				strandSpecificSubgraphs[0] = new DirectedSubgraph<LightweightGenomicAnnotation, BubbleEdge>(this,component,subedges);
				// the [1] is the negative subgraph
				strandSpecificSubgraphs[1] = new DirectedSubgraph<LightweightGenomicAnnotation, BubbleEdge>(this,component,subedges);				
				 ...
				 */
				for(DirectedSubgraph<LightweightGenomicAnnotation, BubbleEdge> ds: strandSpecificSubgraphs){
					 ...
					 */
					Collection<LightweightGenomicAnnotation> subSources = getSourceVertices(getValidTypes(),ds);	
					Collection<LightweightGenomicAnnotation> subSinks = getSinkVertices(getValidTypes(),ds);
					Collection<Path> subgraphPaths = new TreeSet<Path> ();
					/*
					 *  	FOR EACH SOURCE IN SUBGRAPH
					 */
					for(LightweightGenomicAnnotation s : subSources ) {
						//logger.trace("Getting paths starting in " + s.toUCSC() + " ");
						Collection<LightweightGenomicAnnotation> targets = findReachableTargetVertices(s, (new ArrayList<LightweightGenomicAnnotation>()), subSinks, (new ArrayList<LightweightGenomicAnnotation>()),0, ds);
						//logger.trace("to " + targets.size() + " targets ");
						long start=System.currentTimeMillis();
						for(LightweightGenomicAnnotation e : targets ) {
	 ...
	 * @return
	 */
	private DirectedSubgraph<LightweightGenomicAnnotation, BubbleEdge> filterOutLessFreqEdges(DirectedSubgraph<LightweightGenomicAnnotation,BubbleEdge> g, double alpha, Collection<EdgeSourceType> validTypes) {
		//For every node in the graph
		for(LightweightGenomicAnnotation vertex : g.vertexSet()) {
	 ...
	 * @return
	 */
	public Set<BubbleEdge> getEdges(Collection<LightweightGenomicAnnotation> vertices){
	...
	public Collection<LightweightGenomicAnnotation> findReachableTargetVertices(LightweightGenomicAnnotation source, Collection<LightweightGenomicAnnotation> alreadyTraversed,Collection<LightweightGenomicAnnotation> sinkVertices, Collection<LightweightGenomicAnnotation> reachableSinks, int counter, DirectedGraph<LightweightGenomicAnnotation, BubbleEdge> g) {
		if(counter>150){
		...
		//if(outOrientedDegree(source, validTypes) == 0 && inOrientedDegree(source, validTypes) >= 1 && !isSpanningGap(source.getStart(), source.getEnd())){
			...
			if(forwardEdges.size() > 0) {
				for(BubbleEdge e : forwardEdges) {
					LightweightGenomicAnnotation opposite = getOppositeVertex(source, e);
	 ...
	 * @return
	 */
	public List<Path> getPaths(LightweightGenomicAnnotation startNode, LightweightGenomicAnnotation endNode, double alpha, double minSpliceFreq, DirectedGraph<LightweightGenomicAnnotation, BubbleEdge> g){
	 ...
	 * @return
	 */
	public List<Path> getPaths(LightweightGenomicAnnotation startNode, LightweightGenomicAnnotation endNode,double alpha, Collection<EdgeSourceType> validEdgeTypes, double minSpliceFreq, DirectedGraph<LightweightGenomicAnnotation, BubbleEdge> g) {
		return getPaths(startNode, endNode, alpha, validEdgeTypes, new HashMap<LightweightGenomicAnnotation, Collection<Path>>(),1, 0, minSpliceFreq, g);
	 ...
	 * @return
	 */
	private List<Path> getPaths(LightweightGenomicAnnotation startNode, LightweightGenomicAnnotation endNode,double alpha, Collection<EdgeSourceType> validEdgeTypes, Map<LightweightGenomicAnnotation, Collection<Path>> nodesVisited, int predictedPathsSoFar, int direction, double minFreq, DirectedGraph<LightweightGenomicAnnotation, BubbleEdge> g) {
		...
		KShortestPaths<LightweightGenomicAnnotation,BubbleEdge> KPaths = new KShortestPaths<LightweightGenomicAnnotation,BubbleEdge>(g,startNode,MAX_GENE_PATHS,nMaxHops);
	...
	private List<Path> PWrapper(List<GraphPath<LightweightGenomicAnnotation,BubbleEdge>> paths){
		
		List<Path> pw = new ArrayList<Path>();
		for(GraphPath<LightweightGenomicAnnotation,BubbleEdge> p :paths){
			Path x = new Path();
	 ...
	 * @return
	 */
	public LightweightGenomicAnnotation getOppositeVertex(LightweightGenomicAnnotation v, BubbleEdge e) {
		
		LightweightGenomicAnnotation source = this.getEdgeSource(e);
		LightweightGenomicAnnotation target = this.getEdgeTarget(e);
	        if (v.equals(source)) {
	            return target;
	...
	public Collection<RefSeqGene> getPaths(int start, int end, double alpha, Collection<EdgeSourceType> validTypes) {
		...
		while(verticesOverlappingIt.hasNext()) {
			...
			if(v.getEnd() != end && !canBeReachedFrom(v, verticesTraversedList)) {
				...
				for(BubbleEdge edge : edges) {
					LightweightGenomicAnnotation nextV = getOppositeVertex(v, edge);//getOpposite(v, edge);
	...
	public Collection<RefSeqGene> getBackwardPaths(int start, int end, double alpha, Collection<EdgeSourceType> validTypes) {
		...
		while(vertexIt.hasNext() ) {
			...
			if(v.getStart() != start && !canBeReachedFrom(v, verticesTraversedList)) {
				...
				for(BubbleEdge edge : edges) {
					LightweightGenomicAnnotation prevV = getOppositeVertex(v, edge);
	 ...
	 */
	 public Collection<BubbleEdge> getIncidentEdges(LightweightGenomicAnnotation vertex){
	  ...
	  */
	 public Collection<LightweightGenomicAnnotation> getSourceVertices(Collection<EdgeSourceType> validTypes,DirectedGraph<LightweightGenomicAnnotation, BubbleEdge> g) {
		 	//GET ALL VERTICES
			Collection<LightweightGenomicAnnotation> vertices = g.vertexSet();
	  ...
	  * @return All vertices having allowed edge types.
	  */
	 public Collection<LightweightGenomicAnnotation> getSinkVertices(Collection<EdgeSourceType> validTypes,DirectedGraph<LightweightGenomicAnnotation, BubbleEdge> g) {
		 	
		 	//GET ALL VERTICES
			Collection<LightweightGenomicAnnotation> vertices = g.vertexSet();
			Collection<LightweightGenomicAnnotation> targets = new TreeSet<LightweightGenomicAnnotation>();
			//FOR EACH VERTEX
			for(LightweightGenomicAnnotation v: vertices) {
				//System.err.println(v.toUCSC()+" "+inOrientedDegree(v, validTypes)+" "+outOrientedDegree(v, validTypes) + " " + isSpanningGap(v.getStart(), v.getEnd()));
				/*
	...
	public Collection<BubbleEdge> getForwardEdges(LightweightGenomicAnnotation vertex, double alpha, Collection<EdgeSourceType> validTypes) {
	 ...
	 public Collection<BubbleEdge> getValidOutgoingEdges(LightweightGenomicAnnotation vertex, double alpha, Collection<EdgeSourceType> validTypes, DirectedGraph<LightweightGenomicAnnotation, BubbleEdge> g) {
	  ...
	  */
	 public int inOrientedDegree(LightweightGenomicAnnotation v, Collection<EdgeSourceType> validTypes,DirectedGraph<LightweightGenomicAnnotation, BubbleEdge> g) {
			int inDegree = 0;
	  ...
	  */ 
	public int outOrientedDegree(LightweightGenomicAnnotation v, Collection<EdgeSourceType> validTypes, DirectedGraph<LightweightGenomicAnnotation, BubbleEdge> g) {
		int outDegree = 0;
	...
	public void addEdgeAddingMissingVertices(LightweightGenomicAnnotation intron, float count) {
		...
		BubbleEdge edge = intron.inReversedOrientation() ? getEdge(end, start) : getEdge(start, end);
		if(edge != null) {
	 ...
	 * @return
	 */
	public VertexPair<LightweightGenomicAnnotation> getNodePair(BubbleEdge edge) {
		return (new VertexPair<LightweightGenomicAnnotation>(edge.getLeftNode(),edge.getRightNode()));
	}
	...
	public void setLocalRate(Collection<Path> paths){
		this.localRate=new TreeMap<Alignments,Double>();
		for(Path path: paths){
	...
	//	IntervalTree<LightweightGenomicAnnotation> nodeTree=this.vertices;
		...
		for(BubbleEdge edge: edges){
			VertexPair<LightweightGenomicAnnotation> nodes=getNodePair(edge);
	...
	public static class BubbleEdge implements Comparable<BubbleEdge>{
		...
		public BubbleEdge(LightweightGenomicAnnotation connection, LightweightGenomicAnnotation v1, LightweightGenomicAnnotation v2, double connectionCount, EdgeSourceType type) {
			...
			if(this.connection.getOrientation()==null){
				this.connection.setOrientation("*");
			}
		}
		 ...
		 * @param startVertex
		 */
		public BubbleEdge(LightweightGenomicAnnotation startVertex) {
		 ...
		 */
		public void setVertexCount(LightweightGenomicAnnotation vertex, double i) {
		 ...
		 */
		public double getVertexCount(LightweightGenomicAnnotation vertex){
