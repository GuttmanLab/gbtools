...
public class AlignmentDataModelStats {
	...
	private int getTotalSplicedReads() throws IOException{
		...
		for(String chr: data.getChromosomeLengths().keySet()){
			Alignments chrRegion=new Alignments(chr, 0, data.getChromosomeLength(chr));
	...
	public Collection<Alignments> getOverlappingRegions(String chr, Collection<Alignments> introns) throws IOException {		
		...
		//int counter=0;
		while(data.hasNextExon(chr)){
			if(current==null){current=data.getNextExon(chr);}
			Alignments exonic=current;
			while(data.hasNextExon(chr)){
				Alignments next=data.getNextExon(chr);
				if(next!=null && current.overlapsAtAll(next)){
					exonic=exonic.merge(next);
					current=next;
				}
				else{current=next; break;}
			}
			if(exonic!=null){rtrn.add(exonic);}
		}
		
		data.restartIterator();
		rtrn.addAll(introns);
	...
	public int[] getGeneCounts(Collection<RefSeqGene> genes,  int extensionFactor) throws IOException{
		int exon=0;
		int intron=0;
		int UTR5=0;
		int UTR3=0;
	 ...
	 * @throws IOException
	 */
	public int[] getGeneCountsWithoutDatasetTotals(Collection<RefSeqGene> genes,  int extensionFactor) throws IOException{
		int exon=0;
		int intron=0;
		int UTR5=0;
		int UTR3=0;
	...
	public Map<RefSeqGene, double[]> getGeneExpression(Collection<RefSeqGene> genes, int extensionFactor) throws IOException {
	...
	public int getCounts(Collection<Alignments> alignments, int extensionFactor) throws IOException {
		// JE 9/26/12 Use caching in GenericAlignmentDataModel
		
		int rtrn = 0;
		for (Alignments a : alignments) {
			rtrn += data.getCountsPerAlignment(a, extensionFactor);  // converting double to int here ...
		}
		return rtrn;
	}

	public double getCountsPerAlignment(Alignments align, IntervalTree<Alignment> tree, int extensionFactor) {
		return data.getCountsPerAlignment(align, tree, extensionFactor);
	}

	public double getCountsPerAlignment(RefSeqGene window,IntervalTree<Alignment> tree, int extensionFactor) {
		return data.getCountsPerAlignment(window, tree, extensionFactor);
	}

	public double getCountsPerAlignment(Alignments align,Map<String, IntervalTree<Alignments>> exonTree,IntervalTree<Alignment> tree, int extensionFactor) {
	...
	public Map<Alignments, Integer> getSplicedReads(Alignments chrRegion,int i, int j) throws IOException {
		return data.getSplicedReads(chrRegion, i, j);
	}

	public Map<Alignments, Integer> getSplicedReads(Alignments region,List<ReadFilter> filters, int minNumIntrons) throws IOException {
		return data.getSplicedReads(region, filters, minNumIntrons);
	}

	public Map<Alignments, Integer> getSplicedReads(Alignments region,int minIntronSize, int maxIntronSize, int minNumberOfSplices, Sequence chrSequence) throws IOException {
	...
	public Iterator<Alignments> getExonAlignmentsOverlappingRegion(Alignments align) throws IOException {
		return data.getExonAlignmentsOverlappingRegion(align);
	}

	public CloseableIterator<Alignment> getAlignmentsOverlappingRegion(	Alignments alignments) throws IOException {
		
		return data.getAlignmentsOverlappingRegion(alignments);
	}

	public double getCountsOfUniqueOverlappers(Alignments startPosition,
			Alignments window, IntervalTree<Alignment> chunkAlignmentTree,
			int extensionFactor) {
	...
	public double[] scanPRate(RefSeqGene gene, int extensionFactor) throws IOException {
	...
	public double [] scanRegion(int fixedWidth,  LightweightGenomicAnnotation region) throws IOException{
		...
		for(int i=region.getStart(); i<until; i++){
			...
			Alignments current=new Alignments(region.getChromosome(), start, end);
			//System.err.println("Current: " + current.toUCSC());
			double sum=0;
	
			if(score==0){
				sum=data.getCountsPerAlignment(current, chunkAlignmentTree,0); 
				//System.err.println("Computed sum from alignments: " + sum);
				if(sum ==0) {
					i=start+fixedWidth;
	...
	public double[] scanPRate(RefSeqGene gene, IntervalTree<Alignment> tree,int extensionFactor) throws IOException {
		String chr=gene.getChr();
		return scanPRate(gene, tree, getLambda(chr), extensionFactor);
	}
	
	public double[] scanPRate(Alignments align, IntervalTree<Alignment> tree, double localLambda, int extensionFactor) throws IOException{

		return scanPRate(align, data.getCountsPerAlignment(align, tree, extensionFactor), localLambda, extensionFactor);
	}
	
	public double[] scanPRate(LightweightGenomicAnnotation align, double readCount, int extenstionFactor) throws IOException{
		return scanPRate(align, readCount, getLambda(align.getChromosome()), extenstionFactor);
	}
	
	public double[] scanPRate(LightweightGenomicAnnotation align, double readCount, double localLambda, int extenstionFactor) throws IOException{
	...
	public double[] scanPRate(RefSeqGene gene, IntervalTree<Alignment> tree,double localLambda, int extensionFactor) throws IOException {
		String chr=gene.getChr();
	...
	public double[] scanPRate(RefSeqGene gene, IntervalTree<Alignment> tree,double localLambda, int extensionFactor, double numMarkers) throws IOException {
		String chr=gene.getChr();
	...
	public double count(Alignments align, int extensionFactor) throws IOException {
	...
	public int countWithinExon(Alignments align, int extensionFactor) throws IOException {
		//IntervalTree<Alignment> tree=data.getIntervalTreeCached(align.getChr(), align.getStart(), align.getEnd());
	...
	public boolean passes(RefSeqGene window,IntervalTree<Alignment> chunkAlignmentTree, int i, int critVal) {
