...
public abstract class AnnotationReader <T extends GenomicAnnotation> implements AlignmentCollection {
	...
	/**
	 * alternative is to use getAnnotationList
	 * @return
	 
	public Iterator<T> iterator() {
		return new AnnotationReaderIterator();
	}
	
	/**
	 * @author engreitz
	 * Iterator to go through all the annotations in annotationSetMap i.
	 * 
	 * TODO:  BUT I don't think this works properly yet
	 * @param <T>
	 
	public class AnnotationReaderIterator<T extends GenomicAnnotation> implements Iterator<T> {
		int i;
		Iterator<String> chrItr = getChromosomeIterator();
		Iterator<T> annotationItr = null;
		T nextAnnotation = null;;
		
		public AnnotationReaderIterator() {
			this(0);
			advance();
		}
		
		public AnnotationReaderIterator(int i) {
			this.i = i;
		}
		...
		private void advance() {
			if (nextAnnotation == null) {
				if (chrItr.hasNext()) {
					IntervalTree<T> chrTree = (IntervalTree<T>) annotationSetMap.get(i).getAnnotationTree(chrItr.next());
					annotationItr = new IntervalTree.ValuesIterator<T>(chrTree.iterator());
				}
			}
			
			if (annotationItr.hasNext()) {
				nextAnnotation = annotationItr.next();
			} else {
				nextAnnotation = null;
			}
		}
	...
	*/
	 ...
	 */
	public BasicAnnotationReader takeComplement(List<? extends GenomicAnnotation> annotations) {
		
		BasicAnnotationReader complement = new BasicAnnotationReader();
		for (GenomicAnnotation a: annotations) {
			List<T> overlapers = getOverlappers(a);
			List<GenomicAnnotation> annotationComplement = a.minus(overlapers);
			for(GenomicAnnotation ac : annotationComplement) {
				complement.addAnnotation(complement.createAnnotation(ac));
			}
		}
		complement.merge();
		return complement;
	...
	public void filterByOverlap(List<? extends GenomicAnnotation> annotations, int extensionFactor) {
		HashMap<String, IntervalTree<T>> intersectTree = new HashMap<String, IntervalTree<T>>();
		Iterator<? extends GenomicAnnotation> it = annotations.iterator();
	...
	public void filterByOverlapOfLastExonOrAfterLastExon(List<? extends GenomicAnnotation> annotations, int extensionFactor) {
		...
		while(it.hasNext()) {
			...
			List<? extends GenomicAnnotation> annotBlocks = annot.getBlocks();
			Iterator<? extends GenomicAnnotation> annotBlocksIt = annotBlocks.iterator();
			while (annotBlocksIt.hasNext()) {
				GenomicAnnotation block = annotBlocksIt.next();
	...
	public void filterByOverlapAndOrientation(List<? extends GenomicAnnotation> annotations) {
		...
		while(it.hasNext()) {
			...
			while(overlaperIt.hasNext()) {
				...
				if (newTree.find(overlapperNode.getStart(), overlapperNode.getEnd()) == null && overlapper.getOrientation().equalsIgnoreCase(annot.getOrientation())) {
					newTree.put(overlapper.getStart(), overlapper.getEnd(), overlapper);
				}
			}
		}
	...
	public static Map<GenomicAnnotation, Integer> getLastExons(List<? extends GenomicAnnotation> annotations) {
		Map<GenomicAnnotation, Integer> lastExons = new HashMap<GenomicAnnotation, Integer>();
		
		Iterator<? extends GenomicAnnotation> it = annotations.iterator();
		while(it.hasNext()) {
			GenomicAnnotation annot = it.next();
			
			GenomicAnnotation lastExon = null;
			int annotEnd = annot.getOrientedEnd();
	...
	public List<T> getOverlappers(List<? extends GenomicAnnotation> annotationList) {
		List<T> overlappers = new ArrayList<T>();
		Iterator<? extends GenomicAnnotation> annotationIt = annotationList.iterator();
		while(annotationIt.hasNext()) {
	...
	public List<T> getOverlappers(AnnotationReader<? extends GenomicAnnotation> other) {
		return getOverlappers(other.getAnnotationList());
	}
	
	/* (non-Javadoc)
	 * @see broad.pda.seq.slide.AlignmentCollection#getCount(broad.pda.datastructures.Alignments)
	 */
	@Override
	public int getCount(Alignments region) throws IOException {
		return getOverlappers(region).size();
	}
	
	/* (non-Javadoc)
	 * @see broad.pda.seq.slide.AlignmentCollection#getBasesCovered(broad.pda.datastructures.Alignments)
	 * Pretty bad implementation.  TODO use merge instead
	 * Jesse
	 */
	@Override
	...
	public int getBasesCovered(Alignments region, int EF) throws IOException {
		...
		for (int i=0;i<region.getSize();i++) covered[i] = false;

		Iterator<T> iter = getOverlappers(region).iterator();
		while (iter.hasNext()) {
			T record = iter.next();
			record.setStart(record.getStart() - EF);
			record.setEnd(record.getEnd() + EF);
			
			int startIndex = Math.max(0, record.getStart() - region.getStart());
			int endIndex = Math.min(region.getSize(), record.getEnd() - region.getStart());
