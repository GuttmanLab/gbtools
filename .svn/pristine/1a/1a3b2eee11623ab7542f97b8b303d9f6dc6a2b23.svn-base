...
public class geneExpUtils {
	...
	private static void CalcNeighborsCorrelation(String set1bed,
		...
		//Find all neighbors
		List <RefSeqGene> genes= new LinkedList();
		List <RefSeqGene> neighbors= new LinkedList();
		
		List <RefSeqGene> set1Lst =set1.GetGenes();
		for (RefSeqGene g: set1Lst){
		...
		*/
		...
		for (int i=0;i<genes.size();i++){
			RefSeqGene g=genes.get(i);
			RefSeqGene n=neighbors.get(i);
	...
	private static void getLeftNeighbors(RefSeqGene g, BEDFileParser set2,
			List<RefSeqGene> genes, List<RefSeqGene> neighbors) {

		IntervalTree<RefSeqGeneWithIsoforms> tree=set2.getChrTree(g.getChr());  
		Node<RefSeqGeneWithIsoforms> LeftNode=tree.max(g.getStart(), g.getEnd()-1);
		if (LeftNode != null){
			genes.add(g);
	...
	private static void getRightNeighbors(RefSeqGene g, BEDFileParser set2,
			List<RefSeqGene> genes, List<RefSeqGene> neighbors) {
		IntervalTree<RefSeqGeneWithIsoforms> tree=set2.getChrTree(g.getChr());  
		Node<RefSeqGeneWithIsoforms> RightNode=tree.min(g.getStart()+1, g.getEnd());
		if (RightNode != null){
			genes.add(g);
	...
	private static void makeRPKMmat(String refLociBed, String bedLst,
		...
		for (String setName: setsLst){
			...
			while (chrIt.hasNext()){
				...
				Iterator<RefSeqGeneWithIsoforms> geneIt= ref.getChrTree(chr).valueIterator();
				while (geneIt.hasNext()){
					RefSeqGeneWithIsoforms loci =geneIt.next();
	...
	private static void CalcGlobalRpkmFromScriptureOutput(String set1,BufferedWriter bw) throws IOException {
		...
		while (chrIt.hasNext()){
			String chr=chrIt.next();
			Iterator<RefSeqGeneWithIsoforms> geneIt= bed.getChrTree(chr).valueIterator();
			while (geneIt.hasNext()){
				RefSeqGeneWithIsoforms loci =geneIt.next();
		...
		while (chrIt.hasNext()){
			...
			while (geneIt.hasNext()){
				RefSeqGeneWithIsoforms loci =geneIt.next();
				for (RefSeqGene g:loci.getAllIsoforms()){
