...
public class EstimateOmegaPerExon {
	...
	public EstimateOmegaPerExon(File BEDFile, File modelFile, String alnDir, String alnFileFormat, String save)throws Exception{
		Set<RefSeqGene> genes=BEDFileParser.loadData(BEDFile);
		writeOmegaPerExon(genes, modelFile, alnDir, alnFileFormat, save);
	}
	
	public EstimateOmegaPerExon(Collection<RefSeqGene> genes, File modelFile, String alnDir, String alnFileFormat, String save)throws Exception{
		writeOmegaPerExon(genes, modelFile, alnDir, alnFileFormat, save);
	}
	
	private void write(Alignments align, OmegaFit fit, FileWriter writer) throws IOException {
	...
	private void writeOmegaPerExon(Collection<RefSeqGene> genes, File modelFile, String alnDir, String alnFileFormat, String save) throws Exception {
		FileWriter writer = new FileWriter(save);
	...
	private Map<Alignments, OmegaFit> computeOmegaPerExon(Collection<RefSeqGene> genes, File modelFile, String alnDir, String alnFileFormat)throws Exception{
		Map<Alignments, OmegaFit> rtrn=new TreeMap<Alignments, OmegaFit>();
		for(RefSeqGene gene: genes){
			//System.err.println(gene);
			String alnFile=alnDir+"/"+gene.getChr()+".maf";
			Collection<Alignments> exons=gene.getSortedAndUniqueExons();
	...
	public static OmegaFit fitOmega(Alignments align, File modelFile, String alnFile, String alnFileFormat)throws Exception{
		TreeScaler scaler=new TreeScaler(align, modelFile, alnFile, alnFileFormat);
		OmegaFit regionFit = scaler.scaleRegion(align);
		return regionFit;
	}
	
	public static ArrayList<OmegaFit> slideWindowComputeOmega(Alignments align, File modelFile, String alnFile, String alnFileFormat, int windowSize, int overlap)throws Exception{
		TreeScaler scaler=new TreeScaler(align, modelFile, alnFile, alnFileFormat);
